...
//Funcion para formatear datos y evitar tomar un string como lista
...
export async function obtenerAtributos(tabla) {
    ...
    const result = await pool.request()
        .query(`SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = ${mapeoTablas[tabla]}`);
...
export async function seleccionarTodos(tabla) {
    const pool = await poolPromise;
    const result = await pool.request()
        .query(`SELECT * FROM ${mapeoTablas[tabla]}`);
...
export async function seleccionarId(tabla, id) {
    ...
    const result = await pool.request()
        .input('id', id)
        .query(`SELECT * FROM ${mapeoTablas[tabla]} WHERE ${atributos[0]} = @id`);
...
export async function insertarDatos(tabla, datos) {
    ...
    try{
        ...
        const query = `INSERT INTO ${mapeoTablas[tabla]} (${columnas}) VALUES (${valores})`;
...
export async function actualizarDatos(tabla, id, datos) {
    ...
    const query = `UPDATE ${mapeoTablas[tabla]} SET ${setClause} WHERE ${atributos[0]} = ${id}`;
...
export async function eliminarDatos(tabla, id) {
    try{
        ...
        const query = `DELETE FROM ${mapeoTablas[tabla]} WHERE ${atributos[0]} = @${atributos[0]}`;
        await request.query(query);
